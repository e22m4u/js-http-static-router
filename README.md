...
Метод возвращает `Promise`, который разрешается логическим значением `true`,
если подходящий файл был найден и процесс его потоковой передачи успешно
инициирован. Если маршрут не совпал, целевой файл физически отсутствует
в директории или метод запроса не поддерживается, возвращается `false`.
